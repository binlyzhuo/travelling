	(function($) {
		...
		$.suggest = function(input, options) {
			...
			function suggest() {
				...
				displayItems(q);
				if(q){
					$results.children('ul').children('li:first-child').each(function(){
						$input.prev().val($(this).attr("rel"));
					});
				}
			}		
			function displayItems(items) {
				...
				if (items=='') {//热门城市遍历
					...
					for(h in options.hot_list){
						html+='<li rel="'+options.hot_list[h][2]+'"><a href="#'+h+'"><span>'+options.hot_list[h][2]+'</span>'+options.hot_list[h][1]+'</a></li>';
					}
					html='<div class="gray ac_result_tip">请输入中文/拼音按↑↓选择</div><ul>'+html+'</ul>';
				}
				else {
					/*if (!items)
					return;
					...
					for (var i = 0; i < options.source.length; i++) {//国内城市匹配
						...
						if (reg.test(options.source[i][0]) || reg.test(options.source[i][1]) || reg.test(options.source[i][2])) {
							html += '<li rel="' + options.source[i][2] + '"><a href="#' + i + '"><span>' + options.source[i][2] + '</span>' + options.source[i][1] + '</a></li>';
						}
					}
					if (html == '') {
			...
			function selectCurrentResult() {
				...
				if ($currentResult) {
					...
					$input.prev().val($currentResult.attr("rel"));
					$results.hide();
