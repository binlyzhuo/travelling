...
select top 500 d.HotelId from T_XC_HotelDescription d where IsIndex=0;
...
d.AreaID as LocationID,d.AreaName as LocationName,d.Longitude,d.Latitude,d.ZoneId,d.ZoneName,d.CtripStarRate,d.CtripUserRate,d.CtripCommRate,d.CommServiceRate,CONVERT(varchar(100), GETDATE(), 112) as CreateDate from T_XC_HotelDescription d
left join T_XC_HotelPrice p on d.HotelID = p.HotelID 
INNER join @hotelid h on h.hotelid = d.HotelID;

select af.Name,af.HotelID from T_XC_HotelAreaInfo af
