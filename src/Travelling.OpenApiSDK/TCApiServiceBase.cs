...
namespace Travelling.OpenApiSDK
{
    ...
    public class TCApiServiceBase
    {
        ...
        /// 账户
        /// </summary>
        private string SceneryTicketAccountID
        {
            get
            {
        ...
        /// 账户密钥
        /// </summary>
        private string SceneryTicketAccountKey
        {
            get
            {
        ...
        private string TCHotelAccount
        {
            get
            {
                return ConfigurationManager.AppSettings["TC_HotelAccount"];
            }
        }

        private string TCHotelAccountKey
        {
            get
            {
                return ConfigurationManager.AppSettings["TC_HotelKEY"];
            }
        }
        ...
        protected TongChengBaseReturnEntity HotelCommentQuery(string reqBody, string serviceName)
        {
            string url = ConfigurationManager.AppSettings["TC_HotelCommentUrl"];
            return SceneryApi(reqBody, serviceName, url);
        }
        /// <summary>
        /// 景区下单相关接口
        ...
        protected TongChengBaseReturnEntity HotelInfoQuery(string reqBody, string serviceName)
        {
            string url = ConfigurationManager.AppSettings["TC_HotelQueryUrl"];
            return HotelApi(reqBody, serviceName, url);
        }
        ...
        private TongChengBaseReturnEntity HotelApi(string reqBody, string serviceName, string url)
        {
            string header = GetRequestHeader(serviceName);
            string reqContent = string.Format("<request>{0}<body>{1}</body></request>", header, reqBody);
            string repContent = HttpCaller.PostDataToServer(url, reqContent);
        ...
        private string GetRequestHeader(string serviceName)
        {
            ...
            ht.Add("AccountID", SceneryTicketAccountID);                                        //API帐户ID，待申请审批通过后发放
            ht.Add("AccountKey", SceneryTicketAccountKey);                                       //API帐户密钥，待申请审批通过后发放
            ...
            reqXml.AppendFormat("<accountID>{0}</accountID>", SceneryTicketAccountID);
