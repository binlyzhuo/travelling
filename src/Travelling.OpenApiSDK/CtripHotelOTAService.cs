...
namespace Travelling.OpenApiSDK
{
    ...
    public class CtripHotelOTAService : CtripBaseApiCall
    {
        ...
        public OTA_HotelSearchReturnEntity OTA_HotelSearch(OTA_HotelSearchCallEntity req)
        {
            ...
            reqXml.AppendFormat("<ns:Criteria AvailableOnlyIndicator=\"{0}\">", req.AvailableOnlyIndicator.ToString().ToLower());
        ...
        public OTA_HotelAvailReturnEntity OTA_HotelAvail(OTA_HotelAvailCallEntity req)
        {
            ...
            xmlDoc.LoadXml(repXml);
            GetHeaderResult(xmlDoc, rep);
            var errorsNodes = xmlDoc.SelectNodes("Response/HotelResponse/OTA_HotelAvailRS/Errors/Error");
            if (errorsNodes != null && errorsNodes.Count>0)
            {
                List<HotelBookErrorInfo> errors = new List<HotelBookErrorInfo>();
                HotelBookErrorInfo errorinfo;
                foreach (XmlElement node in errorsNodes)
                {
                    errorinfo = new HotelBookErrorInfo();
                    errorinfo.Code = node.GetAttribute("Code").ToInt32();
                    errorinfo.Content = node.InnerText;
                    errorinfo.ShortText = node.GetAttribute("ShortText");
                    errorinfo.Type = node.GetAttribute("Type").ToInt32();
                    errors.Add(errorinfo);
                }
                rep.Errors = errors;
            }

            var roomStaysNode = xmlDoc.SelectSingleNode("Response/HotelResponse/OTA_HotelAvailRS/RoomStays");
            if (roomStaysNode != null)
            {
                var roomStayNode = roomStaysNode.SelectSingleNode("RoomStay");
                rep.AvailabilityStatus = ((XmlElement)roomStayNode).GetAttribute("AvailabilityStatus").Trim();
                XmlNode totalNode = roomStayNode.SelectSingleNode("Total");
                if(totalNode!=null)
                {
                    rep.TotalAmountBeforeTax = ((XmlElement)totalNode).GetAttribute("AmountBeforeTax").Trim().ToDecimal();
                    rep.CurrencyCode = ((XmlElement)totalNode).GetAttribute("CurrencyCode").Trim();
                }
                ...
                if(ratePlanNode!=null)
                {
                    ...
                    var mealNode = ratePlanNode.SelectSingleNode("MealsIncluded");
                    if(mealNode!=null)
                    {
                        rep.Breakfast = ((XmlElement)mealNode).GetAttribute("Breakfast");
                    }

                    var roomRateNode = ratePlanNode.SelectSingleNode("RoomRates/RoomRate");
                    if(roomRateNode!=null)
                    {
                        rep.RoomTypeCode = ((XmlElement)roomRateNode).GetAttribute("RoomTypeCode").Trim();
                        rep.RoomTypeName = ((XmlElement)roomRateNode).GetAttribute("RoomType").Trim();

                        var ratesNodes = roomRateNode.SelectNodes("Rates/Rate");
                        if(ratesNodes!=null)
                        {
                            ...
                            foreach (XmlNode n in ratesNodes)
                            {
                                ...
                                roomRate.AmountBeforeTax = ((XmlElement)n.SelectSingleNode("Base")).GetAttribute("AmountBeforeTax").ToDecimal();
                                roomRate.CurrencyCode = ((XmlElement)n.SelectSingleNode("Base")).GetAttribute("CurrencyCode").Trim();
        ...
        public OTA_HotelResReturnEntity OTA_HotelRes(OTA_HotelResCallEntity req)
        {
            ...
            XmlNode CancelAmountNode = HotelReservationNode.SelectSingleNode("ResGlobalInfo/Total");
            if (CancelAmountNode==null)
            {
                rep.CancelAmount = ((XmlElement)CancelAmountNode).GetAttribute("Amount").Trim().ToDecimal();
                rep.CancelCurrencyCode = ((XmlElement)CancelAmountNode).GetAttribute("CurrencyCode").Trim();
            }
            return rep;
        }
