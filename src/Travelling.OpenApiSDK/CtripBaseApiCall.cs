...
namespace Travelling.OpenApiSDK
{
    ...
    public abstract class CtripBaseApiCall
    {
        ...
        protected string TuanApiCall(CtripBaseAPICallEntity requestEntity)
        {
            string requestUrl = string.Format("{0}/Tuan/{1}.asmx", CtripApiUrl, requestEntity.RequestType);
        ...
        protected string UserApiCall(CtripBaseAPICallEntity requestEntity)
        {
            string requestUrl = string.Format("{0}/User/{1}.asmx", CtripApiUrl, requestEntity.RequestType);
            HttpWebRequest request = HttpWebRequest.Create(requestUrl) as HttpWebRequest;
            request.Method = "POST";
            request.ContentType = "application/xml";
            string lang = "ZH-CN";
