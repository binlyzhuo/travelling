/* Travelling.DataLayer 2.0 - PetaPoco v4.0.3.12 - A Tiny ORMish thing for your POCO's.
...
namespace Travelling.DataLayer
{
    public class Database : IDatabase
    {
        ...
        public IEnumerable<T> Query<T>(int top,Sql Sql)
        {
            return Query(default(T),top, Sql);
        }
        ...
        public IEnumerable<T> Query<T>(T instance,int top, Sql Sql)
        {
            var sql = Sql.SQL;
            var args = Sql.Arguments;

            if (EnableAutoSelect) sql = AutoSelectHelper.AddSelectClause<T>(this,top, sql);
        ...
        {
            var pd = PocoDataFactory.ForType(typeof(T));
            Execute(string.Format("truncate table {0}", _dbType.EscapeTableName(pd.TableInfo.TableName)));
        }
