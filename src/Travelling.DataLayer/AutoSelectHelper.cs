...
namespace Travelling.DataLayer
{
    public class AutoSelectHelper
    {
        ...
        public static string AddSelectClause<T>(Database database,int top, string sql)
        {
            if (sql.StartsWith(";"))
                return sql.Substring(1);
            ...
            if (!rxSelect.IsMatch(sql))
            {
                ...
                string cols = String.Join(", ", columns.ToArray());
                if (!rxFrom.IsMatch(sql))
                    sql = String.Format("SELECT top {3} {0} FROM {1} {2}", cols, tableName, sql,top);
                else
                    sql = String.Format("SELECT top {2} {0} {1}", cols, sql,top);
            }
            return sql;
        }
    }
